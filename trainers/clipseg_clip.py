...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'CLIPSeg',
                      "vision_depth": 0,
                      "language_depth": 0, 
                      "vision_ctx": 0,
                      "language_ctx": 0}
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, text, num_samples=30, return_clip_loss=True):
        ...
        tokenized_prompts = torch.cat([
            clip.tokenize(t, context_length=self.context_length, truncate=True) for t in text
        ...
        if self.training:
            ...
            return seg_logits, clip_aux_loss, None
