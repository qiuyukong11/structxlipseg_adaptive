...
class CustomCLIP(nn.Module):
        ...
        if structure_image is not None and (self.lambda_scribble_text != 0 or self.lambda_rgb_scribble != 0):
            ...
            if has_org_s.any():
                ...
                if self.lambda_scribble_text != 0:
                    ...
                    loss_s_t = clip_loss(self.logit_scale.exp().detach() * (img @ txt.t()))
                    ...
                    for b in range(bs):
                        ...
                        for s, e in chunk_token_spans:
                            if 0 <= s < e <= text_tokens_b.shape[0]:
                                with torch.no_grad():
                                    w = F.softmax(text_tokens_b[s:e].norm(dim=-1), dim=0)
                        ...
                        chunk_embeds = chunk_embeds @ self.text_model.text_projection
                        chunk_embeds_n = F.normalize(chunk_embeds, dim=-1)
                        ...
                        kb = edge_valid_mask[b].to(device=device).bool()
                        if kb.any():
                            idx_start = b * k
    ...
    def forward(
        ...
        if self.training:
            ...
            # Original CLIPSeg auxiliary loss: keep this calculation unchanged.
