...
SNAPSHOT_OUTPUT="output_latest_cpu_snapshot"
...
CKPT_NAME="${DATASET_DIR}_Seg_structxlip_ViT-B-16_latest.pth"
LOG_PATH="run_logs/test_latest_cpu_snapshot.log"

mkdir -p "run_logs"
exec > "${LOG_PATH}" 2>&1

echo "[start] $(date)"
echo "[mode] CPU-only latest checkpoint test"
