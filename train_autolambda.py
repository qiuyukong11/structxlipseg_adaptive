...
class StructWeightController:
    def __init__(self, cfg, device):
        ...
        self.num_tasks = len(STRUCT_TASKS)
    ...
    def _loss_vector(self, raw_losses):
        losses = []
        ref = next((raw_losses.get(key) for key in STRUCT_LOSS_KEYS if torch.is_tensor(raw_losses.get(key))), None)
        if ref is None:
            raise RuntimeError("StructWeightController expected StructXLIP raw loss tensors")
        zero = ref.new_zeros(())
        for key in STRUCT_LOSS_KEYS:
    ...
    def compute(self, raw_losses):
        ...
        diagnostics.update({
            ...
            "weight_st": effective_weights[0],
            "weight_rs": effective_weights[1],
            "weight_chunk": effective_weights[2],
            "log_sigma_st": log_sigma[0],
            "log_sigma_rs": log_sigma[1],
            "log_sigma_chunk": log_sigma[2],
            "autol_meta_grad_st": self.last_meta_grad[0],
            "autol_meta_grad_rs": self.last_meta_grad[1],
            "autol_meta_grad_chunk": self.last_meta_grad[2],
            "dwa_ratio_st": self.last_dwa_ratio[0],
            "dwa_ratio_rs": self.last_dwa_ratio[1],
            "dwa_ratio_chunk": self.last_dwa_ratio[2],
        })
...
def _train_objective_for_autolambda(model, controller, batch, cfg, ce_loss, dice_loss, use_clip_loss, clip_loss_weight):
    ...
    raw_losses = controller._loss_vector(data["raw_struct_losses"])
    weights = controller.meta_weights.to(raw_losses.device, dtype=raw_losses.dtype)
    return data["main_loss"] + (weights * raw_losses).sum()
...
def main():
    ...
    if cfg.MODEL.CLIP_MODEL == "structxlip":
        cfg.DATASET.NAME = (
            ...
            + f"_autolambda_{cfg.weight_method}_{cfg.grad_method}"
        )
    ...
    logger.info(f"StructXLIP weight method: {controller.method}; grad method: {cfg.grad_method}")
    ...
    if cfg.MODEL.CLIP_MODEL == "structxlip" and start_epoch == 0:
        ...
        logger.info(f"StructXLIP Auto-Lambda diagnostics CSV: {diagnostics_path}")
    ...
    for epoch in range(start_epoch, cfg.TRAIN.NUM_EPOCHS):
        ...
        for i, batch in enumerate(tqdm(train_dataloader)):
            ...
            weighted_struct_loss, struct_loss_terms, struct_diag_tensors = controller.compute(raw_struct_losses)
            loss = data["main_loss"] + weighted_struct_loss
            ...
            grad_loss_terms = [data["main_loss"]] + struct_loss_terms
        ...
        controller.finish_epoch([mean_epoch_loss_st, mean_epoch_loss_rs, mean_epoch_loss_chunk_align])
        ...
        if cfg.MODEL.CLIP_MODEL == "structxlip":
            log_msg += (
                ...
                f"Weights st/rs/chunk: {struct_diag_means.get('weight_st', 0.0):.4f}/"
